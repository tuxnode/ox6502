...
impl<B: Bus> Cpu<B> {
    pub fn step(&mut self) -> u8 {
        ...
        match opcode {
            ...
            opcodes::LDA_ABSX => { let (b, a) = self.absolute_x(); self.lda(a); 4 + self.crossed_page(b, a) as u8 }
            opcodes::LDA_ABSY => { let (b, a) = self.absolute_y(); self.lda(a); 4 + self.crossed_page(b, a) as u8 }
            ...
            opcodes::LDA_AIY => { let (b, a) = self.post_indexed_y(); self.lda(a); 5 + self.crossed_page(b, a) as u8 }
            ...
            opcodes::LDX_ABSY => { let (b, a) = self.absolute_y(); self.ldx(a); 4 + self.crossed_page(b, a) as u8 }
            ...
            opcodes::LDY_ABSX => { let (b, a) = self.absolute_x(); self.ldy(a); 4 + self.crossed_page(b, a) as u8 }
            ...
            opcodes::STA_ABSX => { let (_, a) = self.absolute_x(); self.sta(a); 5 }
            opcodes::STA_ABSY => { let (_, a) = self.absolute_y(); self.sta(a); 5 }
            opcodes::STA_ZPXI => { let a = self.pre_indexed_x(); self.sta(a); 6 }
            opcodes::STA_AIY => { let (_, a) = self.post_indexed_y(); self.sta(a); 6 }
            ...
            opcodes::STZ_ABS => { let (_, addr) = self.absolute_x(); let v = self.y & ((addr >> 8) as u8).wrapping_add(1); self.write(addr, v); 5 } // NMOS: SHY
            opcodes::STZ_ABSX => { let (_, addr) = self.absolute_y(); let v = self.x & ((addr >> 8) as u8).wrapping_add(1); self.write(addr, v); 5 } // NMOS: SHX
            ...
            opcodes::INC_ABSX => { let (_, a) = self.absolute_x(); self.inc_mem(a); 7 }
            ...
            opcodes::DEC_ABSX => { let (_, a) = self.absolute_x(); self.dec_mem(a); 7 }
            ...
            opcodes::CMP_ABSX => { let (b, a) = self.absolute_x(); let v = self.read(a); self.compare(self.a, v); 4 + self.crossed_page(b, a) as u8 }
            opcodes::CMP_ABSY => { let (b, a) = self.absolute_y(); let v = self.read(a); self.compare(self.a, v); 4 + self.crossed_page(b, a) as u8 }
            opcodes::CMP_ZPXI => { let a = self.pre_indexed_x(); let v = self.read(a); self.compare(self.a, v); 6 }
            opcodes::CMP_AIY => { let (b, a) = self.post_indexed_y(); let v = self.read(a); self.compare(self.a, v); 5 + self.crossed_page(b, a) as u8 }
            ...
            opcodes::AND_ABSX => { let (b, a) = self.absolute_x(); let v = self.read(a); self.a &= v; self.update_nz(self.a); 4 + self.crossed_page(b, a) as u8 }
            opcodes::AND_ABSY => { let (b, a) = self.absolute_y(); let v = self.read(a); self.a &= v; self.update_nz(self.a); 4 + self.crossed_page(b, a) as u8 }
            opcodes::AND_ZPXI => { let a = self.pre_indexed_x(); let v = self.read(a); self.a &= v; self.update_nz(self.a); 6 }
            opcodes::AND_AIY => { let (b, a) = self.post_indexed_y(); let v = self.read(a); self.a &= v; self.update_nz(self.a); 5 + self.crossed_page(b, a) as u8 }
            ...
            opcodes::ORA_ABSX => { let (b, a) = self.absolute_x(); let v = self.read(a); self.a |= v; self.update_nz(self.a); 4 + self.crossed_page(b, a) as u8 }
            opcodes::ORA_ABSY => { let (b, a) = self.absolute_y(); let v = self.read(a); self.a |= v; self.update_nz(self.a); 4 + self.crossed_page(b, a) as u8 }
            opcodes::ORA_ZPXI => { let a = self.pre_indexed_x(); let v = self.read(a); self.a |= v; self.update_nz(self.a); 6 }
            opcodes::ORA_AIY => { let (b, a) = self.post_indexed_y(); let v = self.read(a); self.a |= v; self.update_nz(self.a); 5 + self.crossed_page(b, a) as u8 }
            ...
            opcodes::EOR_ABSX => { let (b, a) = self.absolute_x(); let v = self.read(a); self.a ^= v; self.update_nz(self.a); 4 + self.crossed_page(b, a) as u8 }
            opcodes::EOR_ABSY => { let (b, a) = self.absolute_y(); let v = self.read(a); self.a ^= v; self.update_nz(self.a); 4 + self.crossed_page(b, a) as u8 }
            opcodes::EOR_ZPXI => { let a = self.pre_indexed_x(); let v = self.read(a); self.a ^= v; self.update_nz(self.a); 6 }
            opcodes::EOR_AIY => { let (b, a) = self.post_indexed_y(); let v = self.read(a); self.a ^= v; self.update_nz(self.a); 5 + self.crossed_page(b, a) as u8 }
            ...
            opcodes::BIT_ABSX => { let (_, _) = self.absolute_x(); 4 } // NMOS: DOP abs,x (NOP)
            ...
            opcodes::ASL_ABSX => { let (_, a) = self.absolute_x(); self.asl_mem(a); 7 }
            ...
            opcodes::LSR_ABSX => { let (_, a) = self.absolute_x(); self.lsr_mem(a); 7 }
            ...
            opcodes::ROR_ABSX => { let (_, a) = self.absolute_x(); self.ror_mem(a); 7 }
            ...
            0xB3 => { let (b, a) = self.post_indexed_y(); self.lax(a); 5 + self.crossed_page(b, a) as u8 }
            0xB7 => { let a = self.zeropage_y(); self.lax(a); 4 }
            0xBF => { let (b, a) = self.absolute_y(); self.lax(a); 4 + self.crossed_page(b, a) as u8 }
            ...
            // DCP: Decrement memory then Compare with A (RMW — no page cross penalty)
            0xC3 => { let a = self.pre_indexed_x(); self.dcp(a); 8 }
            0xC7 => { let a = self.zeropage(); self.dcp(a); 5 }
            0xCF => { let a = self.absolute(); self.dcp(a); 6 }
            0xD3 => { let (_, a) = self.post_indexed_y(); self.dcp(a); 8 }
            0xD7 => { let a = self.zeropage_x(); self.dcp(a); 6 }
            0xDB => { let (_, a) = self.absolute_y(); self.dcp(a); 7 }
            0xDF => { let (_, a) = self.absolute_x(); self.dcp(a); 7 }

            // ISB/ISC: Increment memory then Subtract with Carry (RMW — no page cross penalty)
            0xE3 => { let a = self.pre_indexed_x(); self.isb(a); 8 }
            0xE7 => { let a = self.zeropage(); self.isb(a); 5 }
            0xEF => { let a = self.absolute(); self.isb(a); 6 }
            0xF3 => { let (_, a) = self.post_indexed_y(); self.isb(a); 8 }
            0xF7 => { let a = self.zeropage_x(); self.isb(a); 6 }
            0xFB => { let (_, a) = self.absolute_y(); self.isb(a); 7 }
            0xFF => { let (_, a) = self.absolute_x(); self.isb(a); 7 }

            // SLO: ASL memory then ORA with A (RMW — no page cross penalty)
            0x03 => { let a = self.pre_indexed_x(); self.slo(a); 8 }
            0x07 => { let a = self.zeropage(); self.slo(a); 5 }
            0x0F => { let a = self.absolute(); self.slo(a); 6 }
            0x13 => { let (_, a) = self.post_indexed_y(); self.slo(a); 8 }
            0x17 => { let a = self.zeropage_x(); self.slo(a); 6 }
            0x1B => { let (_, a) = self.absolute_y(); self.slo(a); 7 }
            0x1F => { let (_, a) = self.absolute_x(); self.slo(a); 7 }

            // RLA: ROL memory then AND with A (RMW — no page cross penalty)
            0x23 => { let a = self.pre_indexed_x(); self.rla(a); 8 }
            0x27 => { let a = self.zeropage(); self.rla(a); 5 }
            0x2F => { let a = self.absolute(); self.rla(a); 6 }
            0x33 => { let (_, a) = self.post_indexed_y(); self.rla(a); 8 }
            0x37 => { let a = self.zeropage_x(); self.rla(a); 6 }
            0x3B => { let (_, a) = self.absolute_y(); self.rla(a); 7 }
            0x3F => { let (_, a) = self.absolute_x(); self.rla(a); 7 }

            // SRE: LSR memory then EOR with A (RMW — no page cross penalty)
            0x43 => { let a = self.pre_indexed_x(); self.sre(a); 8 }
            0x47 => { let a = self.zeropage(); self.sre(a); 5 }
            0x4F => { let a = self.absolute(); self.sre(a); 6 }
            0x53 => { let (_, a) = self.post_indexed_y(); self.sre(a); 8 }
            0x57 => { let a = self.zeropage_x(); self.sre(a); 6 }
            0x5B => { let (_, a) = self.absolute_y(); self.sre(a); 7 }
            0x5F => { let (_, a) = self.absolute_x(); self.sre(a); 7 }

            // RRA: ROR memory then ADC (RMW — no page cross penalty)
            0x63 => { let a = self.pre_indexed_x(); self.rra(a); 8 }
            0x67 => { let a = self.zeropage(); self.rra(a); 5 }
            0x6F => { let a = self.absolute(); self.rra(a); 6 }
            0x73 => { let (_, a) = self.post_indexed_y(); self.rra(a); 8 }
            0x77 => { let a = self.zeropage_x(); self.rra(a); 6 }
            0x7B => { let (_, a) = self.absolute_y(); self.rra(a); 7 }
            0x7F => { let (_, a) = self.absolute_x(); self.rra(a); 7 }

            // AHX/SHX: Store A AND X AND (high byte of addr + 1) — write, no penalty
            0x93 => { let (_, addr) = self.post_indexed_y(); let v = self.a & self.x & ((addr >> 8) as u8).wrapping_add(1); self.write(addr, v); 6 }
            0x9F => { let (_, addr) = self.absolute_y(); let v = self.a & self.x & ((addr >> 8) as u8).wrapping_add(1); self.write(addr, v); 5 }

            // TAS: Transfer A AND X to SP, then store high byte — write, no penalty
            0x9B => {
            ...
            // LAS: Load A, X, and SP from memory — read, page cross penalty applies
            0xBB => {
                let (b, addr) = self.absolute_y();
                let v = self.read(addr) & self.sp;
                self.a = v;
                self.x = v;
                self.sp = v;
                self.update_nz(v);
                4 + self.crossed_page(b, addr) as u8
            }
    ...
    // Branch Helper — returns cycles
    fn branch(&mut self, condition: bool) -> u8 {
        let old_pc = self.pc;
        let offset = self.fetch() as i8;
        if condition {
            self.pc = self.pc.wrapping_add(offset as u16);
            if self.crossed_page(old_pc, self.pc) { 4 } else { 3 }
        } else {
            2
        }
    }
