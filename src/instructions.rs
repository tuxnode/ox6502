...
impl<B: Bus> Cpu<B> {
    pub fn step(&mut self) -> u8 {
        ...
        match opcode {
            ...
            opcodes::LDA_ABSX => {
                let (b, a) = self.absolute_x();
                self.lda(a);
                4 + self.crossed_page(b, a) as u8
            }
            opcodes::LDA_ABSY => {
                let (b, a) = self.absolute_y();
                self.lda(a);
                4 + self.crossed_page(b, a) as u8
            }
            ...
            opcodes::LDA_ZPXI => {
                let a = self.pre_indexed_x();
                self.lda(a);
                6
            }
            opcodes::LDA_AIY => {
                let (b, a) = self.post_indexed_y();
                self.lda(a);
                5 + self.crossed_page(b, a) as u8
            }
            ...
            opcodes::STA_ABSX => {
                let (_, a) = self.absolute_x();
                self.sta(a);
                5
            }
            opcodes::STA_ABSY => {
                let (_, a) = self.absolute_y();
                self.sta(a);
                5
            }
            opcodes::STA_ZPXI => {
            ...
            opcodes::ROR_A => {
                let c = self.a & 0x01 != 0;
                self.a = (self.a >> 1) | ((self.get_flag(FLAG_C) as u8) * 0x80);
                self.set_flag(FLAG_C, c);
                self.update_nz(self.a);
                2
            }
            ...
            0x93 => {
                let (_, addr) = self.post_indexed_y();
    ...
    fn branch(&mut self, condition: bool) -> u8 {
        ...
        if condition {
            ...
            if self.crossed_page(old_pc, self.pc) {
                4
            } else {
                3
            }
    ...
    fn adc(&mut self, val: u8) {
        ...
        self.set_flag(
            FLAG_V,
            (((self.a ^ val) & 0x80) == 0) && (((self.a ^ result as u8) & 0x80) != 0),
        );
        self.a = result as u8;
        self.update_nz(self.a);
    }
